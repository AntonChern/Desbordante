...
using VertexImage = std::unordered_map<vertex_t, std::set<vertex_t>>;
...
bool CheckToken(Token const& token, std::string& tok, graph_t const& graph, graph_t const& pattern,
    ...
    if (token.first == -1) {
        ...
        std::map<std::string, std::string> const& attrs = graph[v].attributes;
        if (auto it = attrs.find(token.second); it != attrs.end()) {
            tok = it->second;
            return true;
        }
        return false;
...
bool Satisfied(std::vector<Literal> const& literals, graph_t const& graph, graph_t const& pattern,
               Embedding const& embedding) {
    return std::ranges::all_of(literals, [&](auto const& l) {
...
std::size_t GetMinSize(VertexImage const& vertex_map, std::size_t fallback_size) {
    return vertex_map.empty()
                   ? fallback_size
                   : std::ranges::min(vertex_map | std::views::transform([](auto const& map) {
...
void PopulateVertexMap(MapType& vertex_map, graph_t const& pattern,
                       EmbeddingType const& embedding) {
    BGL_FORALL_VERTICES_T(v, pattern, graph_t) {
        auto [it, emplaced] = vertex_map.try_emplace(v, std::set<vertex_t>{embedding.at(v)});
        if (!emplaced) {
...
bool Validate(graph_t const& graph, Gfd const& gfd, Embeddings const& embeddings,
    ...
    std::size_t min_size = GetMinSize(vertex_map, boost::num_vertices(graph));
...
bool IsForbidden(Rules const& forbidden_rules, std::vector<Literal> const& subset,
                 Literal const& l) {
    return std::ranges::any_of(forbidden_rules, [&subset, &l](auto const& rule) {
        auto const& [premises, conclusion] = rule;
        return CompareLiteralSets(premises, subset) && ContainsLiteral(conclusion, l);
    });
}
...
void HGenerate(std::vector<Literal>& notsatisfied, Literal const& l, std::vector<SingleRule>& rules,
               ...
               Rules const& forbidden_rules, std::size_t sigma) {
    ...
    for (std::size_t k = 1; k <= 3; ++k) {
        ...
        for (auto& subset : subsets) {
            ...
            if (Validate(graph, {pattern, subset, conclusion}, embeddings, sigma)) {
                rules.emplace_back(subset, l);
                auto cond = [&subset](Literal const& lit) {
                    return std::ranges::find(subset, lit) != subset.end();
                };
                notsatisfied.erase(std::ranges::remove_if(notsatisfied, cond).begin(),
                                   notsatisfied.end());
            }
        }
    }
...
std::vector<SingleRule> GenerateRules(std::vector<Literal> const& literals, graph_t const& graph,
    ...
    for (std::size_t j = 0; j < literals.size(); ++j) {
        ...
        if (Validate(graph, {pattern, std::vector<Literal>{}, std::vector<Literal>{l}}, embeddings,
                     sigma)) {
            rules.push_back({{}, l});
            continue;
        }
        HGenerate(current, l, rules, graph, pattern, embeddings, forbidden_rules, sigma);
    }
    return rules;
}
...
void AddCompacted(std::vector<SingleRule>& rules, std::vector<Gfd>& result, graph_t& pattern,
    ...
    for (auto const& [premises, l] : rules) {
        if (prev != premises) {
            result.emplace_back(pattern, prev, conclusion);
...
void UpdateExisted(bool& isoExists, graph_t const& new_pattern,
    ...
    for (auto const& existed : new_patterns) {
        auto vcompare = [&new_pattern, &existed](vertex_t const& fr, vertex_t const& to) {
            return new_pattern[fr].attributes.at("label") == existed[to].attributes.at("label");
        };
        auto ecompare = [&new_pattern, &existed](edge_t const& fr, edge_t const& to) {
            return new_pattern[fr].label == existed[to].label;
        };
        Embedding iso;
        auto callback = [&new_pattern, &iso, &isoExists](auto f, auto) {
            BGL_FORALL_VERTICES_T(u, new_pattern, graph_t) {
                iso.emplace(u, get(f, u));
            }
            isoExists = true;
            return false;
        };
        using property_map_type = boost::property_map<graph_t, boost::vertex_index_t>::type;
        property_map_type new_index_map = get(boost::vertex_index, new_pattern);
        property_map_type existed_index_map = get(boost::vertex_index, existed);
        std::vector<vertex_t> new_vertex_order = vertex_order_by_mult(new_pattern);
        ...
        for (auto& [premises, conclusion] : new_forbidden_rules) {
            ...
            bool rule_exists =
                    std::ranges::any_of(rules, [&premises, &conclusion](auto const& existing_rule) {
                        auto& [new_premises, new_conclusion] = existing_rule;
                        return CompareLiteralSets(premises, new_premises) &&
...
void TryAddPattern(graph_t const& base_pattern, Embeddings const& embeddings,
    ...
    // auto result = embeddings | std::views::transform(embedding_transform) | std::views::join;
    // std::ranges::copy(result, std::back_inserter(new_embeddings));
...
void AddEdge(std::set<std::string> const& edge_labels, graph_t const& graph, graph_t const& pattern,
    ...
    auto it = std::ranges::find_if(pairs, [&pattern](auto const& pair) {
        vertex_t u = boost::vertex(pair[0], pattern);
        vertex_t v = boost::vertex(pair[1], pattern);
    ...
    auto embedding_transform = [&origin, &finish, &graph](Embedding const& embedding) {
        return boost::edge(embedding.at(origin), embedding.at(finish), graph).second
                       ? std::vector<Embedding>{embedding}
                       : std::vector<Embedding>{};
    };

    for (auto& label : edge_labels) {
        auto modify_pattern = [&origin, &finish, &label](graph_t& new_pattern) {
        ...
        for (auto const& vertex_label : vertex_labels) {
            for (std::size_t j = 0; j < num_pat_vertices; ++j) {
                ...
                auto modify_pattern = [&u, &num_pat_vertices, &vertex_label, &edge_label,
                                       &curr_vertex](graph_t& new_pattern) {
                    std::map<std::string, std::string> attributes = {{"label", vertex_label}};
                    curr_vertex = boost::add_vertex(
