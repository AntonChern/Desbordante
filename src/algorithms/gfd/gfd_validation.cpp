...
Eigen::MatrixXi GFDValidation::convert(const Eigen::VectorXi& vec, const int& cols) {
...
std::vector<Eigen::VectorXi> GFDValidation::getCandidateMatches(const Pattern& query,
        const Pattern& graph, const std::pair<int, int>& link) {
...
std::vector<Eigen::VectorXi> GFDValidation::getMatches(const Pattern& query, const Pattern& graph,
        const std::pair<int, int>& link) {
    std::vector<Eigen::VectorXi> result = {};
    std::vector<Eigen::VectorXi> candidates = getCandidateMatches(query, graph, link);
    for (const Eigen::VectorXi& match : candidates) {
        if (isSubgraph(query, graph, convert(match, graph.getAdjacencyMatrix().rows()))) {
...
bool GFDValidation::isSatisfiedLinked(const Graph& graph, const GFD& gfd,
        const std::pair<int, int>& link) {
    std::vector<Eigen::VectorXi> matches = getMatches(gfd.getPattern(), graph, link);
    for (const Eigen::VectorXi& match : matches) {
        ...
        for (const Literal& l : gfd.getPremises()) {
            ...
            std::map<std::string, std::string> current_attrs1 = graph.getAttributes().at(graph_index1);
            if (vars.size() == 1) {
                // ConstLiteral
                ...
                std::map<std::string, std::string> current_attrs2 = graph.getAttributes().at(graph_index2);
                if ((current_attrs1.find(values.at(0)) != current_attrs1.end()) &&
                    (current_attrs2.find(values.at(1)) != current_attrs2.end()) &&
                    (current_attrs1.at(values.at(0)) != current_attrs2.at(values.at(1)))) {
                    // next match
                    satisfied = false;
                    break;
                }
...
std::vector<std::vector<int>> GFDValidation::balanced(const std::vector<int>& weights, const int& m) {
...
Graph GFDValidation::getSubgraph(const Graph& graph, const int& index, const int& radius, int& out) {
    ...
    for (const auto& edge_label : graph.getEdges()) {
        ...
        if ((needed_vertices.find(edge.first) != needed_vertices.end()) &&
            ...
            edges.emplace(std::pair<int, int>(isomorphism.at(edge.first), isomorphism.at(edge.second)),
                edge_label.second);
        }
    }
...
std::vector<int> GFDValidation::getCandidateVertices(const Pattern& pattern, const std::string& label,
    ...
    for (const auto& vertex_label : pattern.getVertices()) {
        ...
        if ((G.row(index).sum() >= degrees.first) && (G.col(index).sum() >= degrees.second)) {
...
void GFDValidation::calculateUnsatisfied(const std::vector<std::tuple<int, int, int, int>>& messages,
        const std::map<int, GFD>& coded_gfds, const std::map<int, Graph>& coded_subgraphs, std::set<int>& out) {
    for (const auto& message : messages) {
        ...
        if (!isSatisfiedLinked(coded_subgraphs.at(subgraph_index), coded_gfds.at(gfd_index),
...
std::vector<GFD> GFDValidation::getSatisfiedGFDs(const Graph& graph, const std::vector<GFD>& gfds, const int& m) {
    ...
    for (int i = 0; i < m; ++i) {
        ...
        for (int& weight : balanced_weights.at(i)) {
            std::tuple<int, int, int, int> temp = *(--weighted_messages.at(weight).end());
    ...
    for (int i = 0; i < m; ++i) {
        std::thread thrd(&GFDValidation::calculateUnsatisfied, this,
            std::cref(groups.at(i)), std::cref(coded_gfds), std::cref(coded_subgraphs), std::ref(answers.at(i)));
...
unsigned long long GFDValidation::Execute() {
