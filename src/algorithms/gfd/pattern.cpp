...
Pattern::Pattern(const std::map<int, std::string>& vertices_, const std::map<std::pair<int, int>, std::string>& edges_) noexcept(false) {
    int max = 0;
...
bool Pattern::operator==(const Pattern& other) const {
    ...
    Eigen::MatrixXi match = Eigen::MatrixXi::Zero(this->vertexNum, this->vertexNum);
    for (int i = 0; i < this->vertexNum; ++i) {
...
void Pattern::print() const {
    std::cout << "Adjacency matrix:" << std::endl << this->adjacencyMatrix << std::endl;
    ...
    for (const auto& edge_label : this->edges) {
        ...
        std::cout << "("  << edge.first << "," << edge.second << "):" << edge_label.second << "; ";
    }
}
